...
# Getting Started
This repository contains the ScoscheSDK24 Framework. This framework may be used to integrate Scosche Rhythm 24 and Rhythm+ activity sensor data into iOS apps.
...
* Physical iOS 12.0 & above device with Bluetooth to test (Note: Simulator does not have a BLE radio, and cannot be used to test communication)
...
```
After modifying the podfile, open Terminal and type the following commands:
...
Open the `Demo.xcworkspace` file that has been generated by cocoapods. The SDK24 framework and its dependencies will be added to the pod folder. 
...
Once a sensor is connected, the SDK24 starts listening for BLE advertisements and routes this data to any `ScoscheViewController`. The BLE data is collected and available as public vars within this view type. The `ScoscheViewController` view is an extension of UIViewController and can be used to create any views needed to display sensor data. 
...
Below is a list of data that can be used in any ScoscheViewController by passing in `monitor: ScoscheMonitor = chosen sensor`. Monitor is a shared object and can be set internally or globally as needed.
```
/// Sport modes
...
/// ScoscheGetAgeInMonths: Age is stored in Scosche devices as months. This function transforms Date of Birth to months.
///
